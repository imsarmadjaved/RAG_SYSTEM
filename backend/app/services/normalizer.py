...
class Normalizer:
    def __init__(self):
        self.model = genai.GenerativeModel(settings.OPENAI_MODEL_EXTRACTION)
    ...
    async def _normalize(self, chunk):
        ...
        prompt = f"""Extract data from this resume [{ctype}] section. Return ONLY JSON.
    ...
    def _format(self, data, ctype):
        ...
        if data.get('experience'):
            for exp in data['experience']:
                if isinstance(exp, dict): parts.append(f"{exp.get('title','')} at {exp.get('company','')} ({exp.get('duration_years',0)}y)")
