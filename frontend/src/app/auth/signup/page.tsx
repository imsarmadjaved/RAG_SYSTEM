...
import { FileText, Mail, Lock, User, Briefcase, Eye, EyeOff, UserPlus } from "lucide-react";

export default function SignupPage() {
  const router = useRouter();
  const { signup } = useAuth();
  const [form, setForm] = useState({ email: "", password: "", full_name: "", user_type: "candidate", company: "" });
        ...
        <div className="glass rounded-2xl p-8 border border-white/10">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type={showPwd ? "text" : "password"} required value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })}
                  className="w-full pl-11 pr-12 py-3 bg-white/5 border border-white/10 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-white placeholder-gray-500 outline-none transition-all" placeholder="Min 8 chars" />
            ...
            <div>
              ...
              <div className="relative">
                <Briefcase className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500" size={18} />
                <select value={form.user_type} onChange={(e) => setForm({ ...form, user_type: e.target.value })}
                  ...
                  <option value="candidate" className="bg-slate-800">Candidate - Looking for job</option>
                  <option value="recruiter" className="bg-slate-800">Recruiter - Hiring</option>
                </select>
              </div>
            </div>
